...
public:
  ...
  Frame(int frame_size, int local_count, temp::Label *name,
        std::list<frame::Access *> *formals)
      : outgo_count(frame_size), local_count_(local_count), name_(name),
        formals_(formals)
  ...
  virtual tree::Exp *ExternalCall(std::string_view s,
                                   tree::ExpList *args) = 0;
