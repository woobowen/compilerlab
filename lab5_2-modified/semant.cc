...
type::Ty *CallExp::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  if (actual_count < formal_count) {
    errormsg->Error(pos_, "too few arg_list in function %s", func_->Name().c_str());
  } else if (actual_count > formal_count) {
    errormsg->Error(pos_, "too many arg_list in function %s", func_->Name().c_str());
  }
...
type::Ty *SeqExp::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  for (auto expr : seq_->GetList())
    res = expr->SemAnalyze(venv, tenv, labelcount, errormsg);
  return res;
}
...
type::Ty *IfExp::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  if (elsee_) {
    ...
    if (!SameType(then_ty, else_ty))
      errormsg->Error(pos_, "then expr and else expr type mismatch");
    return then_ty;
  }
  if (!IsVoid(then_ty))
    errormsg->Error(pos_, "if-then expr's body must produce no value");
...
type::Ty *ForExp::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  if (!IsInt(hi_->SemAnalyze(venv, tenv, labelcount, errormsg)))
    errormsg->Error(hi_->pos_, "for expr's range type is not integer");
  venv->BeginScope();
...
type::Ty *LetExp::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  type::Ty *res = body_->SemAnalyze(venv, tenv, labelcount, errormsg);
  tenv->EndScope();
  venv->EndScope();
  return res;
}
...
type::Ty *AssignExp::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  if (!SameType(exp_ty, var_ty))
    errormsg->Error(pos_, "unmatched assign expr");
  return type::VoidTy::Instance();
}
...
void FunctionDec::SemAnalyze(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  for (auto fun : functions_->GetList()) {
    ...
    type::Ty *res = fun->result_ ? tenv->Look(fun->result_)
                                    : type::VoidTy::Instance();
    if (!res) {
      errormsg->Error(fun->pos_, "undefined type %s",
                      fun->result_->Name().c_str());
      res = type::IntTy::Instance();
    }
