...
public:
  ...
  explicit InFrameAccess(int mem_offset) : mem_offset(mem_offset) {}
...
frame::Frame *NewFrame(temp::Label *name, std::list<bool> formals) {
  ...
  int mem_offset=0;
  // 获取当前机器字长
  int word_sz=reg_manager->WordSize();
  X64Frame *frame=new X64Frame(name, arg_list);
  ...
  for(auto formal:*(frame->Formals()))
  {
    ...
    {
      frame->view_shift=move_stmt; // 可以被删除？
    }
    else
    {
